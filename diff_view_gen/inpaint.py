...
from diff_view_gen.mask_options import mask_proc_options, mask_options
from diff_view_gen.pt3d_mesh_io import load_obj
from diff_view_gen.reproj import render_depth_map, backward_oculusion_aware_render
from diff_view_gen.utils import import_config_key, create_dir, listify_matrix
from diff_view_gen.utils3D import swap_faces, position_verts, init_ngp_config, write_outframe, convert_pt_NGP_transform
...
def initialize_meshes(inpaint_config, mesh_config, device):
    ...
    if mesh_path[-3:] == "obj":
        verts, faces, _ = load_obj(mesh_path, load_textures=False, create_texture_atlas=False, swap_face=swap_face)
...
def inpaint_first_view(meshes, pipe, latents, inpaint_config, mesh_config, device):
    ...
    image = pipe(prompt=view_dep_prompt(prompt_obj, next_angle, color_obj), image=input_image, mask_image=mask,
...
def inpaint_new_angle(
        cur_angle, next_angle, inc_total, inpaint_config, mesh_config, pipe, latents, meshes, bg_image,
        transforms_config_out, angle_inc, device
):
    num_inference_steps = inpaint_config['num_inference_steps']
    save_dir = create_dir(mesh_config["save_dir"])
    dataset_dir = create_dir(f'{save_dir}/dataset')
    ...
    images = backward_oculusion_aware_render(
        ...
        angle_inc=angle_inc, use_train=False, device=device
    )
    ...
    image = pipe(prompt=view_dep_prompt(prompt_obj, next_angle, color_obj), image=input_image, mask_image=mask,
                 depth_map=depth_tensor[None, :, :], negative_prompt=n_propmt, strength=1,
                 num_inference_steps=num_inference_steps, latents=latents, inpainting_strength=inpainting_strength,
...
def inpaint_bidirectional(
        view_1, view_2, view_synth, inpaint_config, mesh_config, pipe, latents, meshes,
        transforms_config_out, bg_image, device
):
    num_inference_steps = inpaint_config['num_inference_steps']
    save_dir = create_dir(mesh_config["save_dir"])
    dataset_dir = create_dir(f'{save_dir}/dataset')
    ...
    images2 = backward_oculusion_aware_render(
        view_2, view_synth, inpaint_config, mesh_config, meshes, bg_image,
        angle_inc=view_synth - view_2, use_train=True, device=device
    )
    ...
    images = backward_oculusion_aware_render(
        view_1, view_synth, inpaint_config, mesh_config, meshes, bg_image,
        angle_inc=view_synth - view_1, use_train=True, device=device
    )
    ...
    mask_combined = torch.maximum(torch.maximum((images[0, :, :, 3]), (images2[0, :, :, 3])),
    ...
    image = pipe(view_dep_prompt(prompt_obj, view_synth, color_obj), image=image_pil, mask_image=mask,
                 depth_map=depth_tensor[None, :, :], negative_prompt=n_propmt, strength=1,
                 num_inference_steps=num_inference_steps, latents=latents, inpainting_strength=inpainting_strength,
                 mask_blend_kernel=mask_blend_kernel, latent_blend_kernel=latent_blend_kernel).images[0]
...
def write_train_transforms(view_1, view_2, view_synth, mesh_config, transforms_config_out, device):
    ...
    def train_outframe(angle):
        ...
        return {"file_dir": file_dir, "transform_matrix":
            listify_matrix(convert_pt_NGP_transform(torch.tensor([0]).to(device), torch.tensor([angle]).to(device)))[0]}
