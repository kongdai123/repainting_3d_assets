...
def paint(mesh_config,inpaint_config, nerf_config):
    ...
    latents = torch.randn((1, 4, 64, 64), generator = rng_randn, dtype=torch.float16,device = device)
    
    transforms_config_out, bg_image = inpaint_first_view(meshes, pipe, latents, inpaint_config, mesh_config, device)
    
    view_1, view_2, transforms_config_out = inpaint_facade(inpaint_config, mesh_config, pipe, latents, meshes, bg_image, transforms_config_out, device)
    ...
    while True:
        ...
        write_train_transforms(view_1, view_2, view_synth, mesh_config, transforms_config_out, device)
        
        nerf_args.record_video = False
        
        train_nerf(nerf_args)
        
        view_1, view_2, view_synth, transforms_config_out = inpaint_bidirectional(view_1, view_2, view_synth, inpaint_config, mesh_config, pipe, latents, meshes, transforms_config_out, bg_image, device)
    ...
    for i in range(360):
        render_silhouette(i,meshes, inpaint_config, mesh_config, size = nerf_args.video_resolution, device = device)
