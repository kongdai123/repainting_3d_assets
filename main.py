...
class ShapeNetDataset:
    def __init__(self, SHAPENET_DIR, SAVE_DIR, device="cpu", dtype=torch.float32):
        ...
        self.trans_tar = torch.tensor(
            [[1, 0, 0], [0, 1, 0], [0, 0, 1]], device=device
        ).to(dtype)
    ...
    def get(
        ...
        if isinstance(up, str) and isinstance(front, str):
            ...
            fron_vec = convert_string_to_torch(
                front, device=self.device, dtype=self.dtype
            )
            right_vec = up_vec.cross(fron_vec)
            trans_ori = torch.cat(
                [right_vec[None,], up_vec[None,], fron_vec[None,]], dim=0
            )
        else:
            trans_ori = torch.tensor(
                [[1, 0, 0], [0, 0, 1], [0, -1, 0]], device=self.device
            ).to(self.dtype)
        trans_tar = torch.tensor(
            [[1, 0, 0], [0, 1, 0], [0, 0, 1]], device=self.device
        ).to(self.dtype)
...
def process_one(dataset, mesh_idx, inpaint_config, nerf_config):
    ...
    try:
        ...
        if "out of memory" in str(e):
            print(
                "| WARNING: ran out of memory, this seems irrecoverable for the process"
            )
            raise e
        print(e)
        import traceback
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_inpaint_path",
        type=str,
        default="./diff_view_gen/config/config_inpaint.json",
    )
    parser.add_argument(
        "--config_nerf_path", type=str, default="./nerf_recon/config/config_nerf.json"
    )
    ...
    if args.range_start is not None:
        ...
        process_items(
            dataset,
            list(range(args.range_start, args.range_end)),
            inpaint_config,
            nerf_config,
        )
    else:
